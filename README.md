...
首次启动按引导允许访问微信应用和应用管理即可。兼容增强需要管理员授权；其他权限由微信分身在使用时自行申请。

## 兼容增强有什么用

兼容增强是可选功能。**如果你只想登录多个微信，直接使用“分身管理”即可，不需要安装增强。**

- **原生多开增强**：让“再开一个微信”更稳定，适合经常同时登录多个微信的用户。
- **保留已撤回消息**：对方撤回消息后，仍可在这台 Mac 上查看原消息。

应用会先检查微信版本。显示“当前版本可以使用”时才能安装；暂不支持时，会直接告诉你应下载哪个版本。安装前会自动备份微信，也可以一键恢复。

## 使用与风险边界

- 本应用只处理用户本人 Mac 上的微信应用副本，不读取聊天内容，也不连接、扫描或控制腾讯服务器。
...
分身多开不需要安装兼容增强，也不要求腾讯原始签名。兼容增强仍需要精确版本配置与可信备份；微信升级后可能需要等待上游适配。
