...
> **封号风险与停止维护说明**
>
> 微信客户端已经开始检测分身多开、插件及非官方客户端，并将相关行为认定为违规。继续使用本软件可能导致微信账号被警告、限制功能，甚至封号。
>
...
- 本项目与腾讯、微信没有关联。
- 微信已将分身多开和非官方客户端列为违规行为，可能导致警告、功能限制或封号。
...
**多开原理**：应用会复制这台 Mac 上已经安装的微信，为副本设置独立身份并重新签名，然后作为单独的 App 安装。macOS 会为每个分身分配独立数据空间，因此它们可以同时运行并分别登录不同账号。普通多开不会修改官方微信。
